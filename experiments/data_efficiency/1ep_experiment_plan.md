...
### Launcher
`experiments/data_efficiency/multi_node_launch.sh` — SSH-based, no Slurm.
...
```bash
bash experiments/data_efficiency/multi_node_launch.sh \
  --nodes "gpu-st-p4d24xlarge-4,gpu-dy-p4d24xlarge-1" \
  --config experiments/data_efficiency/run_1_4b_1ep_dclm.py \
...
- **Variant B on 4 nodes**: gpu-dy-3, gpu-dy-4, gpu-dy-5, gpu-st-4 (free after smoke)
  ...
  bash experiments/data_efficiency/multi_node_launch.sh \
    ...
    --config experiments/data_efficiency/run_1_4b_1ep_code25.py \
...
**To kill a stuck run:**
```bash
bash experiments/data_efficiency/multi_node_kill.sh \
  --nodes "<same nodes>" \
  --config <same config> \
  --yes-i-mean-it
```
