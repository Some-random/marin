...
#   2. Tokenize both new bands via experiments.reasoning_pretraining.code_ladder.data.code_data_lower_tiers
...
cd $MARIN
MARIN_PREFIX=$MARIN/outputs .venv/bin/python -m experiments.reasoning_pretraining.code_ladder.data.code_data_lower_tiers \
    > "$TOK_LOG" 2>&1
TOK_RC=$?
...
.venv/bin/python experiments/reasoning_pretraining/code_ladder/eval/eval_section3.py fill-from-results \
...
" 2>/dev/null)
[ -n "$PALOMA_VAL" ] && .venv/bin/python experiments/reasoning_pretraining/code_ladder/eval/eval_section3.py fill-cell --row "paloma_macro (bpb)" --col "A5-SP" --value "$PALOMA_VAL" 2>&1 | tee -a "$ORCH_LOG"
...
for jp in glob.glob('$GSM_DIR/**/*results*.json', recursive=True):
    ...
    for tn,m in d.get('results',{}).items():
        if tn == '$task' and 'exact_match,strict-match' in m:
            ...
            break
" 2>/dev/null)
    [ -n "$val" ] && .venv/bin/python experiments/reasoning_pretraining/code_ladder/eval/eval_section3.py fill-cell --row "${task}${suffix}" --col "A5-SP" --value "$val" 2>&1 | tee -a "$ORCH_LOG"
done
...
for jp in glob.glob('$ARYABUMI_DIR/**/*results*.json', recursive=True):
    ...
    for tn,m in d.get('results',{}).items():
        if tn == '$task' and 'acc,none' in m:
            ...
            break
" 2>/dev/null)
    [ -n "$val" ] && .venv/bin/python experiments/reasoning_pretraining/code_ladder/eval/eval_section3.py fill-cell --row "${task}[0]" --col "A5-SP" --value "$val" 2>&1 | tee -a "$ORCH_LOG"
done
...
for jp in glob.glob('$QUAC_DIR/**/*results*.json', recursive=True):
    ...
    for tn,m in d.get('results',{}).items():
        if tn == 'quac_first_turn' and 'f1,none' in m:
            ...
            break
" 2>/dev/null)
[ -n "$val" ] && .venv/bin/python experiments/reasoning_pretraining/code_ladder/eval/eval_section3.py fill-cell --row "quac_first_turn[0]" --col "A5-SP" --value "$val" 2>&1 | tee -a "$ORCH_LOG"
...
" 2>/dev/null)
    [ -n "$dclm_val" ] && .venv/bin/python experiments/reasoning_pretraining/code_ladder/eval/eval_section3.py fill-cell --row "dclm_200m_val (bpb)" --col "A5-SP" --value "$dclm_val" 2>&1 | tee -a "$ORCH_LOG"
fi

.venv/bin/python experiments/reasoning_pretraining/code_ladder/eval/eval_section3.py validate 2>&1 | tee -a "$ORCH_LOG"
...
launch_code25b() {
    ...
    nohup $MARIN/experiments/reasoning_pretraining/code_ladder/orchestration/multi_node_launch.sh \
        ...
        --config experiments/reasoning_pretraining/code_ladder/scripts/run_1_4b_code25b.py \
        --run-tag "$tag" \
