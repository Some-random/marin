...
def _collect_sp_nl_shards_with_rows() -> list[tuple[str, int]]:
    ...
    Approximately token-proportional since chunk1/chunk2 have similar
    avg-tokens-per-row. See run_1_4b_c5v4_phase2.py docstring for the full
    audit notes (2026-06-15). Relies on the Hamilton/largest-remainder
    rounding fix in mixture.py:144 to keep the actual per-block bucket
    totals within 0.04 pp of intended.
    """
    import json
...
def _stage2_weights() -> dict[str, float]:
    """90% SP-NL (ROW-PROPORTIONAL across parts, ≈ token-proportional) + 10% (80% code + 20% markup)."""
    w: dict[str, float] = {}
...
if __name__ == "__main__":
    ...
    print(f"  data: stage1 = 100% code+markup, stage2 = 90% SP-NL ({len(SP_NL_SHARDS)} parts, row-proportional ≈ token-proportional) + 10% (80% code + 20% markup)")
    if not _code_components or not _markup_components or not _sp_nl_components:
