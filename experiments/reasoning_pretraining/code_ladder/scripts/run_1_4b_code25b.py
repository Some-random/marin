...
# === Resolve each code source and look up its row + token counts ===
_CODE_SOURCES = [
    ...
    ("se_python_low2",   "c5_25b_se_python_low2"),             # score [2.5, 2.7)
...
_components_with_rows: list[tuple[str, str, int, int]] = []
for name, prefix in _CODE_SOURCES:
    cache = _resolve_cache(prefix)
    rows = _shard_rows(cache)
    tokens = _shard_tokens(cache)
    _components_with_rows.append((name, cache, rows, tokens))

TOTAL_ROWS = sum(rows for _, _, rows, _ in _components_with_rows) or 1
TOTAL_TOKENS_MEASURED = sum(tokens for _, _, _, tokens in _components_with_rows) or 1
...
# Use MEASURED total tokens from .stats.json (no row-tok heuristic).
TOKENS_AVAILABLE = TOTAL_TOKENS_MEASURED
NUM_TRAIN_STEPS = max(1, TOKENS_AVAILABLE // TOKENS_PER_STEP)
...
_code_components = {
    ...
    for name, cache, _, _ in _components_with_rows
}
_code_weights = {
    name: rows / TOTAL_ROWS
    for name, _, rows, _ in _components_with_rows
}
_shard_val_sizes = {name: 0 for name, _, _, _ in _components_with_rows}
...
train_config = TrainLmConfig(
    ...
    trainer=TrainerConfig(
        ...
        tracker=WandbConfig(
            ...
            tags=["1.4b", "code25b", "single-phase", "curated-code", "wd-0.1", f"nodes-{NUM_PROC}"],
            save_code=False,
        ),
        ...
        checkpointer=CheckpointerConfig(
            base_path="checkpoints/1_4b_code25b/",
...
if __name__ == "__main__":
    print("=== 1.4B Code-Only Base (single-phase, curated code) ===")
    ...
    print(f"  data sources ({len(_components_with_rows)} components, row-weighted = token-proportional sampling):")
    for name, cache, rows, tokens in _components_with_rows:
        cache_short = cache.replace(BASE_TOKENIZED + "/", "")
        weight = rows / TOTAL_ROWS
        tok_b = tokens / 1e9
        tpr = tokens / rows if rows else 0
        print(f"    {name:<22} ({tok_b:>4.2f} B, {tpr:>5.0f} tok/row, w={weight*100:>5.2f}%)  ← {cache_short}")
    print(f"  total measured tokens: {TOTAL_TOKENS_MEASURED / 1e9:.2f} B")
