...
# 1.4B C5-v3 PHASE 1 — HALF-DATA variant: code-LM from scratch on HALF of C5-v6's base.
# All 4 sources (SE-Py >=3.0 + Nemotron-CC/UA + Stack-Edu Markdown) scaled to 0.5x
# (~7.7B). The 0.5x rung of the code-budget ladder: 0.5x (this) | 1x C5-v6 (15.4B) |
# 1.6x code25b_clean (24.65B via >=2.7), all continued on the same 15.4B text.
# Same composition + recipe as C5-v6, fixed >=3.0 quality, no repeat.
#
...
# HALF-DATA variant of C5-v6's base: 7,336 steps = 7.70 B trained.
# Mixture weights are fixed, so halving the step count halves EVERY source
# uniformly (each stays well under 1 epoch — no repeat). The 0.5x code-budget
# rung at fixed >=3.0 quality vs C5-v6 (15.4B, 1x).
NUM_TRAIN_STEPS = 7_336
...
train_config = TrainLmConfig(
    ...
    trainer=TrainerConfig(
        ...
        tracker=WandbConfig(
            ...
            tags=["1.4b", "c5v3-half", "phase1-code-only", "clean-code", "code-budget-0.5x", "wd-0.1", f"nodes-{NUM_PROC}"],
        ),
        ...
        checkpointer=CheckpointerConfig(
            base_path="checkpoints/1_4b_c5v3_phase1_half/",
...
if __name__ == "__main__":
    print("=== 1.4B C5-v3 PHASE 1 HALF-DATA (0.5x code budget, ~7.7B, all sources halved) ===")
