...
>
> The key insight for our data efficiency work: hierarchy is what matters, not just complexity class. ww is context-sensitive (same Chomsky level as k-Shuffle Dyck) but hurts because it lacks hierarchical structure. This suggests that for reasoning curricula, we should look for data with nested/recursive structure rather than just "hard" sequences. Also notable: they only test blocked training (all formal tokens first, then NL). Our setup mixes reasoning data with NL data, which might transfer differently -- the paper explicitly flags this as an open question in limitations (Section 9).
>
> </details>
