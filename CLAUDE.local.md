...
- **Use `run_eval_v2.sh`, NOT `eval_intermediate.sh`, for any model going into §3 of EVALUATION.md.** `run_eval_v2.sh <LABEL> <HF_DIR>` is the canonical full v2 suite. It runs all ~30 §3 tasks AND has the multi-GPU code_eval cache fix (per-rank HF_METRICS_CACHE). `eval_intermediate.sh` only covers ~9 task groups (skips lambada, copa, wsc, agieval, gpqa, bbh, mmlu_pro, bigcode humaneval, paloma per-subset, gsm_symbolic, gsm_noop) — using it leaves §3 rows as `—`. **Always check the script covers the §3 task list before launching.**
