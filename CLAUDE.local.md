...
- **paloma needs ONLINE mode (`HF_DATASETS_OFFLINE=0`, `HF_HUB_OFFLINE=0`).** `allenai/paloma` uses a legacy builder-script that's fetched from the Hub on every `load_dataset` call; offline mode kills it with `ConnectionError: OfflineModeIsEnabled`. Other tasks (gsm, v2-suite parquet datasets) are fine offline because their data ships as plain files. `run_paloma_for_model.sh` is now patched to set OFFLINE=0; do not "fix" it back to offline without testing.
- **Runner scripts that swallow per-task failures with `||` will print "ALL DONE" even when every task failed.** Don't trust the `ALL DONE` marker alone — verify at least one `<results_dir>/<subset>/*results*.json` exists. If empty, grep the `.log` for `Traceback|ConnectionError|FAILED|OfflineMode`. Bit me 2026-06-11 with the first paloma launch where all 16 subsets failed but "ALL DONE" still printed.
- **When I fix an infra bug or discover a non-obvious gotcha, immediately update the relevant skill/runbook docs AND CLAUDE.local.md AND memory — without being asked.** Pattern: fix bug → write lesson into the next agent's reading path (skill `.md`, OPS.md, CLAUDE.local.md, memory). Violated 2026-06-11 on the paloma offline fix — only patched the .sh, didn't propagate the lesson until the user said "shouldn't you update the skill?". User should never have to remind me.
